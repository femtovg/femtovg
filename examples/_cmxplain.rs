...
fn main() {
    ...
    // The same six tile positions, but master has no color-matrix filter:
    // the raw source is all an application can draw.
    let tiles: [(f32, f32); 6] = [
        (50.0, 30.0),
        (180.0, 30.0),
        (310.0, 30.0),
    ...
    for (x, y) in tiles {
        let paint = Paint::image(src, x, y, 64.0, 64.0, 0.0, 1.0);
        let mut p = Path::new();
