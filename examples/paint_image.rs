...
use femtovg::{renderer::OpenGl, Canvas, Color, ImageFlags, PixelFormat, Paint, Path, RenderTarget};

enum Shape { Rect, Ellipse, Polar }
    ...
    if let Ok(size) = canvas.image_size(image_id) {
        ...
        for x in 0..(size.0/grid_size) {
            for y in 0..(size.1/grid_size) {
                canvas.clear_rect(
                    (x*grid_size+1) as u32, (y*grid_size+1) as u32,
                    (grid_size-1) as u32, (grid_size-1) as u32,
        ...
        *control_flow = ControlFlow::Poll;
        ...
        match event {
            ...
            Event::RedrawRequested(_) => {
                ...
                canvas.set_size(window_size.width as u32,
                                window_size.height as u32,
                                dpi_factor as f32);
                ...
                canvas.translate(window_size.width  as f32 / 2.0,
                ...
                if let Ok(size) = canvas.image_size(image_id) {
                    ...
                    canvas.translate(60.0 * (t/3.0).cos(),
                                     60.0 * (t/5.0).sin());
                    ...
                    match &shape {
                        Shape::Rect => {
                            path.rect(x - width/2.0, y - height/2.0,
                                      width, height);
                        },
                        Shape::Ellipse => {
                            let rx = width  / 2.0;
                        ...
                        Shape::Polar => {
                            ...
                            for theta in 0..360 {
                                ...
                                let x = x + r * theta.cos();
                                let y = y + r * theta.sin();
                                if path.is_empty() { path.move_to(x, y); }
                    ...
                    // to the path bbox:
                    let a = canvas.transform().inversed()
                        .transform_point(bbox.minx, bbox.miny);
                    let b = canvas.transform().inversed()
